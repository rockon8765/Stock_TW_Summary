...
| # | 比率 | 公式 | 資料來源 |
| ---: | --- | --- | --- |
| 1 | `ROE_TTM` | 稅後淨利 TTM / avg(期初期末股東權益) | incomeQ + bsQ |
| 2 | `ROA_TTM` | 稅後淨利 TTM / avg(期初期末資產總計) | incomeQ + bsQ |
| 3 | 負債比 | 負債總計 / 資產總計（最新季）| bsQ |
| 4 | 流動比 | 流動資產 / 流動負債 | bsQ |
| 5 | 利息保障倍數 | (稅前淨利 + 利息費用) / 利息費用 | incomeQ |
...
### Step 6：欄位名稱驗證（務必於實作前確認）

**風險：** Dottdot API 的真實欄位名可能與 `dottdot-stock-field-inventory.json` 略有出入（例如「設質」相關欄位在 insider structure 280 欄中的精確命名尚未列出）。

**驗證步驟：**
1. 於實作前對每個新 table 執行一次 `curl` 測試（或直接從 browser 查 `/api/v1/tables/{name}/fields`）
2. 挑一支已知股票（例如 `2330`）做實際 query 檢查 JSON 結構
3. 若 field inventory 有落差，以 API 實回傳為準並補回 docs
